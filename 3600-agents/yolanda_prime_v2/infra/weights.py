...
_SPECS: tuple[ParameterSpec, ...] = (
    ...
    # Decision-boundary knobs newly surfaced so CMA-ES can see the search-gate
    # and carpet-veto constants that previously lived as magic numbers in
    # policy.py / lookahead.py. Defaults match the legacy hard-coded values
    # exactly so behavior is unchanged until a new best_weights.json overrides
    # them.
    ParameterSpec("search_gate_base_farm", 0.22, 0.05, 0.50, "B"),
    ParameterSpec("search_gate_slope_farm", 0.22, 0.05, 0.70, "B"),
    ParameterSpec("search_gate_base_nonfarm", 0.35, 0.10, 0.70, "B"),
    ParameterSpec("search_gate_slope_nonfarm", 0.50, 0.15, 0.95, "B"),
    ParameterSpec("carpet_counter_floor_early", 1.0, 0.0, 4.0, "B"),
    ParameterSpec("carpet_counter_floor_late", 2.0, 0.0, 4.0, "B"),
    ParameterSpec("carpet_counter_floor_pressure_gain", 1.5, 0.0, 3.5, "B"),
    ParameterSpec("lookahead_carpet_risk_penalty", 0.6, 0.0, 1.5, "B"),
    # Fast-search shortcut is off by default (Y3_ENABLE_FAST_SEARCH); these two
    # knobs only have gradient when that flag is on, so we park them outside
    # the default ``core`` optimization profile to avoid wasting dimensions.
    ParameterSpec("fast_search_prob_threshold", 0.80, 0.50, 0.98, "Z"),
    ParameterSpec("fast_search_max_carpet_points", 12.0, 4.0, 18.0, "Z"),
...
PARAMETER_PROFILES: dict[str, tuple[str, ...]] = {
    # Tier A+B only. Fast-search lives in tier Z so it never shows up here.
    "core": tuple(spec.name for spec in _SPECS if spec.tier in {"A", "B"}),
    # Core plus phase scalers (tier C). Still excludes fast-search and time D.
    "phase": tuple(spec.name for spec in _SPECS if spec.tier in {"A", "B", "C"}),
    # Everything: core + phase + time (D) + fast-search shortcut (Z).
...
def load_weights(
    *,
    ...
    env_var: str = "YP2_WEIGHTS_JSON",
    weights_file_name: str = "weights.json",
    allow_env: bool = True,
) -> tuple[dict[str, float], str]:
    """Load safe weight overrides for yolanda_prime_v2 and report their source."""

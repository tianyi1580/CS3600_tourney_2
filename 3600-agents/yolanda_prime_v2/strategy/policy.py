...
class PolicyEngine:
    ...
    def __init__(
        ...
        opening_mult_a: float = 1.2,
        opening_mult_b: float = 1.0,
        opening_mult_c: float = 1.5,
        opening_mult_d: float = 0.4,
        opening_mult_f: float = 1.0,
        mid_mult_a: float = 1.0,
        mid_mult_b: float = 0.8,
        mid_mult_c: float = 1.5,
        mid_mult_d: float = 0.8,
        mid_mult_f: float = 1.0,
        late_mult_a: float = 1.5,
        late_mult_b: float = 0.3,
        late_mult_c: float = 0.5,
        late_mult_d: float = 1.2,
        late_mult_f: float = 0.5,
        # Decision-boundary knobs previously hard-coded in this file and in
        # ``lookahead.py``. Defaults below match the legacy magic numbers
        # exactly, so omitting them in ``weights.json`` preserves pre-upgrade
        # behavior. Surfaced here so CMA-ES can tune them via
        # ``PARAMETER_PROFILES['core']``.
        search_gate_base_farm: float = 0.22,
        search_gate_slope_farm: float = 0.22,
        search_gate_base_nonfarm: float = 0.35,
        search_gate_slope_nonfarm: float = 0.50,
        carpet_counter_floor_early: float = 1.0,
        carpet_counter_floor_late: float = 2.0,
        carpet_counter_floor_pressure_gain: float = 1.5,
        lookahead_carpet_risk_penalty: float = 0.6,
    ) -> None:
        ...
        self.opening_mult_a = opening_mult_a
        self.opening_mult_b = opening_mult_b
        self.opening_mult_c = opening_mult_c
        self.opening_mult_d = opening_mult_d
        self.opening_mult_f = opening_mult_f
        self.mid_mult_a = mid_mult_a
        self.mid_mult_b = mid_mult_b
        self.mid_mult_c = mid_mult_c
        self.mid_mult_d = mid_mult_d
        self.mid_mult_f = mid_mult_f
        self.late_mult_a = late_mult_a
        self.late_mult_b = late_mult_b
        self.late_mult_c = late_mult_c
        self.late_mult_d = late_mult_d
        self.late_mult_f = late_mult_f

        self.search_gate_base_farm = search_gate_base_farm
        self.search_gate_slope_farm = search_gate_slope_farm
        self.search_gate_base_nonfarm = search_gate_base_nonfarm
        self.search_gate_slope_nonfarm = search_gate_slope_nonfarm
        self.carpet_counter_floor_early = carpet_counter_floor_early
        self.carpet_counter_floor_late = carpet_counter_floor_late
        self.carpet_counter_floor_pressure_gain = carpet_counter_floor_pressure_gain
        self.lookahead_carpet_risk_penalty = lookahead_carpet_risk_penalty
        ...
        self._current_prime_chain_pressure: float = 0.0
    ...
    def _phase_multipliers(self, phase: str) -> tuple[float, float, float, float, float]:
    ...
    def _carpet_risk_profile(
        ...
        *,
        ...
        """Shared carpet fragility model for both top-level scoring and lookahead."""
        if action.move_type != MoveType.CARPET:
            return {
                "risk": 0.0,
                "reject": False,
        ...
        counter_threat = self._estimate_opponent_best_response(board_after)
        ...
        if exits <= 1:
            ...
            risk += 0.35
        if space_neighbors <= 0:
            risk += 0.8
        elif space_neighbors == 1:
            risk += 0.25
        if immediate_break:
            risk += 1.4
        risk += 1.2 * max(0.0, 0.6 - survival)
        risk += 0.8 * two_ply_break
        if counter_threat >= 10.0:
            risk += 0.8
        elif counter_threat >= 6.0:
            risk += 0.4

        guaranteed = float(CARPET_POINTS_TABLE[min(action.roll_length, 7)])
        recovery_mode = self._recovery_mode(board)
        turns_left = board.player_worker.turns_left
        # Early phase opponents frequently cash 4-5 carpets in immediate
        # response (turning-points report shows repeated -10 swings at t10-t14).
        # Tighten the counter-threat threshold during the opening so we don't
        # walk into a guaranteed counter-cashout.
        counter_floor = guaranteed + (
            self.carpet_counter_floor_early
            if turns_left > 28
            else self.carpet_counter_floor_late
        )
        # Against prime-chain bullies that we are losing to, passing on a
        # carpet means letting them keep extending — widen the acceptance so
        # we cash high-value chains even with a moderate counter risk.
        pressure = float(getattr(self, "_current_prime_chain_pressure", 0.0))
        if pressure > 0.0:
            counter_floor += self.carpet_counter_floor_pressure_gain * pressure
        reject_base = bool(
            action.roll_length >= 3
            and exits <= 1
            and (
                immediate_break
                or two_ply_break >= 0.45
                or survival < 0.35
                or counter_threat >= counter_floor
            )
        )
        # Desperation deficit: relax veto so we can take calculated risks for
        # high-value cashouts. Only keep the hardest objections (a guaranteed
        # immediate break or a counter that strictly exceeds our cashout).
        if recovery_mode == "desperation_deficit":
            reject = bool(
                action.roll_length >= 3
                and exits <= 1
                and (
                    immediate_break
                    and counter_threat >= guaranteed
                )
            )
        else:
            reject = reject_base
        ...
        return {
            ...
            "reject": reject,
            "exits": float(exits),
            "space_neighbors": float(space_neighbors),
            "survival": float(survival),
            "immediate_break": immediate_break,
            "two_ply_break": float(two_ply_break),
            "counter_threat": float(counter_threat),
        }
    ...
    def _no_good_move_fallback(
        ...
        *,
        ...
        if carpets:
            ...
            for m in carpets:
                bd_after = board.forecast_move(m, check_ok=True)
                if bd_after is None:
                    continue
                guaranteed = float(CARPET_POINTS_TABLE[min(m.roll_length, 7)])
                denial = self._denial_carpet_bonus(board, m)
                profile = self._carpet_risk_profile(
                    board,
                    bd_after,
                    m,
                    self._destination(board, m),
                    deep_eval=False,
                )
                survival = float(profile["survival"])
                immediate_break = bool(profile["immediate_break"])
                ...
                scored = (
                    ...
                    + denial
                    + emergency_cashout
                    + (1.0 - survival) * 0.8 * guaranteed
                    - 0.9 * float(profile["risk"])
                )
    ...
    def _apply_score_pressure(board: Board, state: RuntimeState) -> None:
        ...
        state.last_recovery_mode = PolicyEngine._recovery_mode(board)
        if delta < 0:
    ...
    @staticmethod
    def _move_mode(action: Move) -> str:
        return action.move_type.name.lower()

    @staticmethod
    def _recovery_mode(board: Board) -> str:
        delta = board.player_worker.get_points() - board.opponent_worker.get_points()
        turns_left = board.player_worker.turns_left
        if turns_left <= 0:
            return "neutral"
        if delta <= -10 or (delta <= -7 and turns_left <= 12):
            return "desperation_deficit"
        if delta <= -5:
            return "recoverable_deficit"
        return "neutral"

    def _recent_low_progress_cycle_count(self, state: RuntimeState) -> int:
        modes = list(state.recent_modes)
        deltas = list(state.recent_score_deltas)
        if len(modes) < 2 or len(deltas) < 2:
            return 0
        count = 0
        for idx in range(1, min(len(modes), len(deltas))):
            pair = (modes[idx - 1], modes[idx])
            if pair not in {("search", "prime"), ("prime", "search"), ("search", "search")}:
                continue
            delta_gain = deltas[idx] - deltas[idx - 1]
            if delta_gain <= 1.0:
                count += 1
        return count
    ...
    def _search_gate(
        ...
        state: RuntimeState,
        best_search: Move,
        best_search_sv: float,
        best_non_search: Move | None,
        best_non_search_tv: float,
        best_non_search_meta: dict[str, float],
        ...
        score_delta = board.player_worker.get_points() - board.opponent_worker.get_points()
        recovery_mode = self._recovery_mode(board)
        cycle_penalty = 0.18 * self._recent_low_progress_cycle_count(state)

        safe_points = False
        q_best_non = 0.0
        if best_non_search is not None:
            if best_non_search.move_type == MoveType.CARPET:
                q_best_non = float(CARPET_POINTS_TABLE[min(best_non_search.roll_length, 7)])
                if (
                    best_non_search_meta["mobility"] >= 2.0
                    and best_non_search_meta["risk"] <= 1.1
                ):
                    safe_points = True
            elif best_non_search.move_type == MoveType.PRIME:
                q_best_non = 1.0
                bn_key = (
                    best_non_search.move_type,
                    best_non_search.direction,
                    best_non_search.roll_length,
                    best_non_search.search_loc,
                )
                ...
                if best_non_search_meta["mobility"] >= 2.0 and best_non_search_meta["risk"] <= 0.9:
                    safe_points = True

        turn_fraction = board.player_worker.turns_left / 40.0
        lead_adjustment = 0.0
        if score_delta > 5:
            lead_adjustment = 0.1 + 0.02 * score_delta
        elif score_delta < -5:
            lead_adjustment = -0.1 + 0.01 * score_delta
        lead_adjustment = max(-0.4, min(0.6, lead_adjustment))

        if recovery_mode == "desperation_deficit":
            lead_adjustment -= 0.08
        elif recovery_mode == "recoverable_deficit":
            lead_adjustment -= 0.04

        if state.farmable_rat:
            q_best_non *= 0.90
            guess_margin = (
                self.search_gate_base_farm
                + self.search_gate_slope_farm * turn_fraction
                + lead_adjustment
            )
        else:
            guess_margin = (
                self.search_gate_base_nonfarm
                + self.search_gate_slope_nonfarm * turn_fraction
                + lead_adjustment
            )
        ...
        if safe_points:
            guess_margin += 0.15
        guess_margin += cycle_penalty

        # Hysteresis: if we just searched, raise the bar for searching again
        # unless current belief is materially stronger than what we had then.
        # Avoids the search->search loops that the report flagged in losses.
        if state.last_search_confidence > 0.0:
            belief_gain = p1 - state.last_search_confidence
            if belief_gain <= 0.05:
                guess_margin += 0.20
            elif belief_gain <= 0.15:
                guess_margin += 0.08

        # Compounding-desperation signal: if last turn was already in
        # desperation deficit and we are still there, prefer point actions
        # over speculative searches.
        if (
            recovery_mode == "desperation_deficit"
            and state.last_recovery_mode == "desperation_deficit"
            and not safe_points
        ):
            guess_margin += 0.10

        # `best_search_sv` already encodes 6*p1 - 2; previous +0.5*p1 additive
        # double-counted belief and made the gate net more pro-search. The
        # insights call for a tighter belief threshold, so we use sv directly.
        search_value = best_search_sv
        if recovery_mode == "desperation_deficit" and not safe_points:
            guess_margin -= 0.12
        return search_value > (q_best_non + guess_margin)
    ...
    def score_non_search(
        ...
        *,
        ...
        # Bias toward denial / contention when facing a prime-chain bully and
        # we're behind. Empirically those segments show 0% win rate, so a small
        # boost (max +30%) is better than continuing to bleed value.
        pressure = float(getattr(state, "prime_chain_pressure", 0.0))
        if pressure > 0.0:
            denial_carpet *= (1.0 + 0.30 * pressure)
            contention_bonus *= (1.0 + 0.30 * pressure)
            sabotage_bonus *= (1.0 + 0.20 * pressure)
        ...
        carpet_profile = self._carpet_risk_profile(
            board,
            board_after,
            action,
            dest,
        ...
        if bool(carpet_profile["reject"]):
            return float("-inf"), {"mobility": 0.0, "immediate": immediate, "risk": 1.6, "continuation": 0.0}

        score_delta = my_score - opp_score
        recovery_mode = self._recovery_mode(board)
        recovery_bonus = 0.0
        if recovery_mode != "neutral":
            recovery_bonus += max(0.0, immediate) * (0.35 if recovery_mode == "recoverable_deficit" else 0.55)
            recovery_bonus += 0.25 * max(0.0, exit_count - 1)
            if action.move_type == MoveType.PRIME and not self._prime_enables_next_turn_carpet(board, action, min_roll=3):
                recovery_bonus -= 0.8 if recovery_mode == "recoverable_deficit" else 1.2
            if action.move_type == MoveType.CARPET and carpet_profile["risk"] <= 0.9:
                recovery_bonus += 0.5
            if action.move_type == MoveType.CARPET and carpet_profile["risk"] >= 1.5:
                recovery_bonus -= 0.7
        ...
        cycle_penalty = 0.0
        recent_modes = list(state.recent_modes)
        if action.move_type == MoveType.PRIME:
            if len(recent_modes) >= 2 and recent_modes[-1] == "prime" and recent_modes[-2] == "prime":
                if not self._prime_enables_next_turn_carpet(board, action, min_roll=3) and exit_count <= 2:
                    cycle_penalty -= 0.9
            if recent_modes and recent_modes[-1] == "search" and score_delta >= -4:
                cycle_penalty -= 0.5

        # --- Solution D: Lookahead-Refined Scoring ---
        # Penalize moves that leave the opponent with high-value gains (1-ply mini-rollout)
        # Apply to all candidates to ensure ranking stability (§14.1)
        ...
        total = (
            ...
            - 0.1 * opp_best_response  # Reduced to 0.1 to avoid double-counting with Minimax
        ...
        if action.move_type == MoveType.CARPET:
            total += 0.35 * float(carpet_profile["survival"])
            # `effective_f * risk` already captures exit/corridor risk; the
            # carpet_profile adds a more sophisticated chain-survival signal.
            # Keep the second penalty modest to avoid compounding too hard
            # (which previously suppressed medium-risk cashouts and bled into
            # search_gate via meta["risk"]).
            total -= 0.7 * float(carpet_profile["risk"])

        # For meta["risk"] (which feeds _search_gate.safe_points), use max
        # rather than sum so we don't compound the same risk twice when
        # surfacing it back to the search-vs-move decision.
        meta_risk = (
            max(risk, float(carpet_profile["risk"]))
            if action.move_type == MoveType.CARPET
            else risk
        )
    ...
    @staticmethod
    def _detect_prime_chain_pressure(state: RuntimeState, score_delta: float) -> float:
        """0..1 indicator of a prime-chain bully matchup we're losing.

        The insights report shows complete loss runs against `prime_heavy +
        prime_chain` archetypes on multiple seeds. The general adaptation table
        only nudges `effective_f`; this returns a focused pressure factor that
        downstream scoring uses to widen carpet veto thresholds and bias toward
        denial/contention plays.
        """
        if state.observed_turns < 6 or score_delta > -5:
            return 0.0
        typed = [c for c, _ in state.opp_turn_buffer if c is not None]
        if len(typed) < 5:
            return 0.0
        recent = typed[-10:]
        prime_count = sum(1 for c in recent if c == OpponentCategory.PRIME)
        prime_rate = prime_count / len(recent)
        if prime_rate < 0.55:
            return 0.0
        return min(1.0, max(0.0, (prime_rate - 0.55) / 0.30))
    ...
    def _recovery_filter_non_search(
        self,
        board: Board,
        moves: list[Move],
    ) -> list[Move]:
        recovery_mode = self._recovery_mode(board)
        if recovery_mode == "neutral" or not moves:
            return moves

        safe_value: list[Move] = []
        setup_value: list[Move] = []
        neutral: list[Move] = []
        for mv in moves:
            if mv.move_type == MoveType.CARPET and mv.roll_length >= 2:
                ...
                profile = self._carpet_risk_profile(
                    ...
                    bd_after,
                    mv,
                    self._destination(board, mv),
                    deep_eval=False,
                )
                if not bool(profile["reject"]) and float(profile["risk"]) <= 1.2:
                    safe_value.append(mv)
                else:
                    neutral.append(mv)
            elif mv.move_type == MoveType.PRIME:
                if self._prime_enables_next_turn_carpet(board, mv, min_roll=3):
                    setup_value.append(mv)
                else:
                    neutral.append(mv)
            else:
                bd_after = board.forecast_move(mv, check_ok=True)
                if bd_after is None:
                    continue
                exits = self._count_plain_exits(bd_after, self._destination(board, mv))
                if exits >= 2:
                    safe_value.append(mv)
                else:
                    neutral.append(mv)
        ...
        if recovery_mode == "desperation_deficit":
            return safe_value + setup_value + neutral if (safe_value or setup_value) else moves
        # Recoverable deficit: prioritize setup primes that enable a high-value
        # carpet next turn (the report shows `carpet->prime` and `prime->plain`
        # are win-heavier transitions when behind, indicating that staging the
        # next big cashout outperforms grabbing immediate small safe value).
        return setup_value + safe_value + neutral if (safe_value or setup_value) else moves
    ...
    def select_action(
        ...
        if state.enable_opponent_model:
            ...
            score_delta_for_detect = float(
                board.player_worker.get_points() - board.opponent_worker.get_points()
            )
            state.prime_chain_pressure = self._detect_prime_chain_pressure(
                state, score_delta_for_detect
            )
        else:
            state.prime_chain_pressure = 0.0
        self._current_prime_chain_pressure = state.prime_chain_pressure
        ...
        def _persist_return(mv: Move) -> Move:
            ...
            self._current_prime_chain_pressure = 0.0
            ...
            state.recent_modes.append(self._move_mode(mv))
            state.recent_score_deltas.append(
                float(board.player_worker.get_points() - board.opponent_worker.get_points())
            )
            if mv.move_type == MoveType.SEARCH and mv.search_loc is not None:
                state.last_search_confidence = belief.probability_at(mv.search_loc)
            else:
                state.last_search_confidence = 0.0
            return mv
            ...
            for mv in board.get_valid_moves(exclude_search=True):
                if mv.move_type == MoveType.CARPET:
                    ...
                    if bd_after is not None:
                        ...
                        if not bool(profile["reject"]) and float(profile["risk"]) <= 1.0:
                            safe_point_exists = True
                elif mv.move_type == MoveType.PRIME:
                    bd_after = board.forecast_move(mv, check_ok=True)
                    if bd_after is not None and self._count_plain_exits(bd_after, self._destination(board, mv)) >= 2:
                        safe_point_exists = True
            top_k = belief.topk(1)
            if top_k and best_immediate_carpet < self.fast_search_max_carpet_points and not safe_point_exists:
        ...
        non_search_candidates = self._recovery_filter_non_search(board, non_search_candidates)
        ...
        if my_turns > 3:
            if self._search_gate(
                ...
                best_search_sv,
                best_non_search,
                best_non_search_heuristic_tv,
                best_non_search_meta,
                board_after_cache,
            ):
                return _persist_return(best_search)
            return _persist_return(best_non_search if best_non_search else self._best_safe_non_search(board))

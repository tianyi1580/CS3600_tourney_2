...
class Lookahead:
    ...
    def _transition(self, board: Board, mv: Move, is_enemy: bool) -> tuple[float, Board] | None:
        ...
        if mv.move_type == MoveType.CARPET:
            profile = self.policy._carpet_risk_profile(
                board,
                board_after,
                mv,
                dest,
            ...
            # Replace the prior hard prune with a scalar gain penalty so that
            # the opponent's high-value-but-risky carpets remain visible as
            # threats during minimax (an asymmetric prune would inflate our
            # position evaluation by silently dropping their counter cashouts).
            risk_penalty = self.policy.lookahead_carpet_risk_penalty * float(profile["risk"])
            gain = self._immediate_points(mv) - risk_penalty
            return gain, board_after
    ...
    def static_eval_differential(self, board: Board) -> float:
        ...
        our_score += 0.15 * float(
            sum(
                1 for nxt in (
                    loc_after_direction(worker_pos, direction)
                    for direction in _DIRECTIONS
                )
        ...
        opp_score += 0.15 * float(
            sum(
                1 for nxt in (
                    loc_after_direction(opp_pos, direction)
                    for direction in _DIRECTIONS
                )
                if board.is_valid_cell(nxt) and board.get_cell(nxt).name == "SPACE"
            )

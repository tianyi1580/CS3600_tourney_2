...
class PolicyEngine:
    ...
    def _phase_multipliers(phase: str) -> tuple[float, float, float, float, float]:
        ...
        if phase == "opening":
            # Increased mult_a from 0.8 to 1.2 for more early aggression.
            # Reduced mult_c from 1.8 to 1.5 to balance setup vs immediate points.
    ...
    def score_non_search(
        ...
        *,
        ...
        position = (exit_count / 4.0) + 0.3 * centrality * centrality_scale + 0.04 * spatial + 0.12 * chain_potential
        if (

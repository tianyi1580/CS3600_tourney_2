...
class RuntimeState:
    ...
    time_opening_multiplier: float = 1.80
    time_mid_multiplier: float = 1.40
    time_late_multiplier: float = 1.00
    time_opening_cap: float = 8.0
    time_mid_cap: float = 6.0
    time_late_cap: float = 3.0
    ...
    recent_modes: deque[str] = field(
        default_factory=lambda: deque(maxlen=6)
    )
    recent_score_deltas: deque[float] = field(
        default_factory=lambda: deque(maxlen=6)
    )
    last_search_confidence: float = 0.0
    last_recovery_mode: str = "neutral"
    prime_chain_pressure: float = 0.0
    """0..1 indicator that opponent is a prime-chain bully and we're behind; bumps denial/contention weights and widens carpet counter-threat."""

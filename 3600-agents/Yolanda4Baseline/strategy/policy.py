...
class PolicyEngine:
    ...
    def __init__(
        ...
        f: float = 0.75,
        g: float = 0.50,
        opp_response_weight: float = 0.10,
        opening_mults: tuple[float, float, float, float, float] = (0.8, 1.0, 1.8, 0.4, 1.0),
        mid_mults: tuple[float, float, float, float, float] = (1.0, 0.8, 1.5, 0.8, 1.0),
        late_mults: tuple[float, float, float, float, float] = (1.5, 0.3, 0.5, 1.2, 0.5),
        farmable_ev_scale: float = 0.90,
        farmable_margin_base: float = 0.15,
        farmable_margin_slope: float = 0.20,
        nonfarm_margin_base: float = 0.25,
        nonfarm_margin_slope: float = 0.50,
        ...
        self.f = f
        self.g = g
        self.opp_response_weight = opp_response_weight
        self.opening_mults = opening_mults
        self.mid_mults = mid_mults
        self.late_mults = late_mults
        self.farmable_ev_scale = farmable_ev_scale
        self.farmable_margin_base = farmable_margin_base
        self.farmable_margin_slope = farmable_margin_slope
        self.nonfarm_margin_base = nonfarm_margin_base
        self.nonfarm_margin_slope = nonfarm_margin_slope
    ...
    def _phase_multipliers(self, phase: str) -> tuple[float, float, float, float, float]:
        ...
        if phase == "opening":
            return self.opening_mults
        if phase == "mid":
            return self.mid_mults
        return self.late_mults
    ...
    def score_non_search(
        ...
        *,
        ...
        position = (exit_count / 4.0) + 0.3 * centrality * centrality_scale + 0.04 * spatial
        if (
        ...
        total = (
            ...
            - self.opp_response_weight * opp_best_response
            + timing_adj
            + sabotage_bonus
            + anti_oscillation
            + denial_carpet
    ...
    def select_action(
        ...
        if my_turns > 3 and filtered_non_search_candidates:
            # Hard-cap the tree budget so minimax remains a tactical layer
            # instead of burning the tournament clock.
            lookahead_budget = min(
                max(0.0, (turn_deadline - time.perf_counter()) * 0.75),
                6,  # absolute cap in seconds
            )
            searcher = Expectiminimax(
                self,
                snap_root,
                chain_potential_weight=1.0,
                root_belief=belief.belief.copy(),
                transition_matrix=belief.transition_matrix,
                root_board=board,
                eval_potential_w=self.eval_potential_w,
                eval_chain_w=self.eval_chain_w,
                eval_exit_w=self.eval_exit_w,
                eval_entry_adv_w=self.eval_entry_adv_w,
                eval_trap_w=self.eval_trap_w,
                eval_rat_w=self.eval_rat_w,
                rat_nearby_w=self.rat_nearby_w,
                rat_peak_w=self.rat_peak_w,
                rat_peak_thresh=self.rat_peak_thresh,
            )
        ...
        if my_turns > 3:
            ...
            if state.farmable_rat:
                q_best_non *= self.farmable_ev_scale
                guess_margin = self.farmable_margin_base + self.farmable_margin_slope * turn_fraction
            else:
                guess_margin = self.nonfarm_margin_base + self.nonfarm_margin_slope * turn_fraction

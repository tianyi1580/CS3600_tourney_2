...
class PlayerAgent:
    ...
    def commentate(self):
        rs = self.runtime_state
        return (
            "Yolanda_Mitchell (Promoted Yolanda3.1): Deep search (d6) + carpet-first + memoized rollout "
    ...
    def play(
        ...
        try:
            ...
            if isinstance(sensor_data, tuple) and len(sensor_data) == 2:
                ...
                if noise is not None and estimated_distance is not None:
            ...
            # Time-aware search relaxation
            now_time = time_left()
            time_factor = min(1.5, now_time / 120.0)
            if time_factor > 1.0:
                # If we have plenty of time, we can afford slightly riskier searches
                self.runtime_state.search_threshold_relaxation = 0.5
            else:
                self.runtime_state.search_threshold_relaxation = 0.0

...
from .constants import CARPET_PRIORITY_BONUS, OPENING_PERPENDICULAR_PENALTY, MAX_ENDGAME_SEARCH_RISK
...
class PolicyEngine:
    ...
    def _carpet_intercept_detection(self, board: Board, action: Move, analysis: BoardAnalysis) -> float:
        """
        Detects if the opponent can intercept the intended carpet path.
        Returns a significant penalty if an intercept is possible.
        """
        if action.move_type != MoveType.CARPET:
            return 0.0
        ...
        # Simulate the roll path cell-by-cell
        cur = src
        for roll_step in range(1, action.roll_length + 1):
            cur = loc_after_direction(cur, action.direction)
            
            # Opponent distance to this path cell
            # Manually calculate L1 distance as an approximation of turns to reach
            opp_dist = abs(cur[0] - opp_loc[0]) + abs(cur[1] - opp_loc[1])
            
            # If opponent can reach this cell in <= roll_step turns, they can cut us off.
            # We use <= because the opponent moves AFTER our CARPET move finishes if we succeed,
            # BUT if they are already adjacent to a cell in the path, they might block the 
            # atomic execution if the engine logic considers it (though CARPET is atomic).
            # More importantly, if they can reach the cell we are about to PRIME in the NEXT turn,
            # they can steal it. 
            # In the case of an atomic CARPET roll, they can't interrupt the roll itself,
            # but they can reach the END of the roll or any intermediate cell to block 
            # future moves.
            # User says "primed carpet roll they are about to do is about to get cut off"
            # which implies the opponent reaches a cell in the intended path.
            if opp_dist <= roll_step:
                return -50.0 # Heavy penalty for easily cut-off rolls
    ...
    def _carpet_timing_adjustment(self, board: Board, action: Move, turns_left: int) -> float:
        ...
        if ext_cell is not None:
            opp_near_ext = abs(ext_cell[0] - opp_loc[0]) + abs(ext_cell[1] - opp_loc[1]) <= 3
        opp_near_src = abs(src[0] - opp_loc[0]) + abs(src[1] - opp_loc[1]) <= 3
        far_end = src
        for _ in range(k):
            far_end = loc_after_direction(far_end, action.direction)
        opp_near_far = abs(far_end[0] - opp_loc[0]) + abs(far_end[1] - opp_loc[1]) <= 2
        opp_threatens_chain = opp_near_ext or opp_near_src or opp_near_far

        if opp_threatens_chain:
            return urgency_bonus + 2.0
        ...
        if k == 2 and turns_left > 2:
            ...
            adj -= 0.5
        elif k == 4 and can_extend and not contested and turns_left > k + 3:
            adj -= 0.3
        elif k == 5:
            if can_extend and turns_left > k + 3 and not contested:
                adj -= 0.3
            else:
                adj += 0.8
        elif k == 6:
    ...
    def score_non_search(
        ...
        *,
        ...
        # Opening perpendicular move penalty
        if phase == "opening":
            opp_loc = board.opponent_worker.get_location()
            src = board.player_worker.get_location()
            if src[1] == opp_loc[1] and action.direction in (Direction.UP, Direction.DOWN):
                pass # vertical is good
            elif src[0] == opp_loc[0] and action.direction in (Direction.LEFT, Direction.RIGHT):
                pass # horizontal is good
            elif src[1] != opp_loc[1] and action.direction in (Direction.LEFT, Direction.RIGHT):
                position -= OPENING_PERPENDICULAR_PENALTY
            elif src[0] != opp_loc[0] and action.direction in (Direction.UP, Direction.DOWN):
                position -= OPENING_PERPENDICULAR_PENALTY
        ...
        if action.move_type == MoveType.CARPET and action.roll_length >= 2:
            carpet_setup += immediate * 0.5
            
        # Primed chain awareness and blocking heuristics
        dest_idx = snap_after.index_from_loc(dest)
        if dest_idx in snap_after.primed_adjacency_map:
            k_primed, d_primed = snap_after.primed_adjacency_map[dest_idx]
            if k_primed >= 3:
                carpet_setup += 1.5
            elif k_primed >= 1:
                carpet_setup += 0.5
        
        # Reward blocking opponent access to primed chains
        opp_dist_to_dest = snap_after.dist_to_opponent_idx[dest_idx]
        if opp_dist_to_dest <= 2 and dest_idx in snap_before.primed_adjacency_map:
             k_opp, _ = snap_before.primed_adjacency_map[dest_idx]
             if k_opp >= 3:
                 carpet_setup += 2.0
        ...
        timing_adj += self._carpet_intercept_detection(board, action, snap_before)
        if (
        ...
        if action.move_type == MoveType.CARPET and action.roll_length >= 3:
            total += CARPET_PRIORITY_BONUS # Balanced boost for high-roll carpets
        elif action.move_type == MoveType.CARPET and action.roll_length >= 2:
            total += 2.0
    ...
    def select_action(
        ...
        alloc, emergency = TimeManager.allocation(board, state, now_time)
        ...
        if not candidates or emergency or now_time <= state.emergency_floor_total:
        ...
        for mv in sorted(non_search_candidates, key=self.move_sort_key):
            ...
            if multi_exit_exists and single_exit_flags.get(mv_key, False):
                if not (mv.move_type == MoveType.CARPET and mv.roll_length >= 2):
                    continue
        ...
        if my_turns > 3:
            ...
            if state.farmable_rat:
                ...
                q_best_non *= 0.90
                guess_margin = 0.25
            guess_threshold = q_best_non + guess_margin - state.search_threshold_relaxation
            
            if 6.0 * p1 - 1.5 >= guess_threshold:
                return _persist_return(best_search)
            return _persist_return(best_non_search if best_non_search else self._best_safe_non_search(board))
        else:
            # Endgame Mode
            ...
            # Must-guess override: If we can't win without a guess, and a guess could win, we must try.
            # EV check is secondary to absolute survival here.
            can_win_non_search = (my_score + m_non_self > e_opp_final)
            if not can_win_non_search:
                # If we are definitely losing, any search with positive probability of winning is worth it.
                m_non_after = self._greed_rollout_m_non(board, my_turns - 1)
                if my_score + 4.0 + m_non_after > e_opp_final:
                     return _persist_return(best_search)
            ...
            m_non_after = self._greed_rollout_m_non(board, my_turns - 1)
            # p1 is prob of success (+4), (1-p1) is prob of failure (-2)
            ...
            # Constraint: Don't guess if we have the lead and the risk of losing it is too high.
            if can_win_non_search and (1.0 - p1) > MAX_ENDGAME_SEARCH_RISK:
                pwin_search = -1.0

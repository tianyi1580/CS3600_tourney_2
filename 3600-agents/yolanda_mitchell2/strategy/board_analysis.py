...
class BoardAnalysis:
    ...
    def __init__(self, board: Board) -> None:
        ...
        self.primed_adjacency_map = {
            idx: (self._entry_best_len[idx], self._entry_best_dir[idx])
            for idx in range(BOARD_CELLS)
            if self._entry_best_len[idx] >= 1 and self._entry_best_dir[idx] is not None
        }
        self.dist_to_opponent_idx = self._dist_matrix[self.opponent_idx * BOARD_CELLS : (self.opponent_idx + 1) * BOARD_CELLS]

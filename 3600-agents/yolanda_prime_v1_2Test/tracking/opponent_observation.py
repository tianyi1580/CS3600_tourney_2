"""
Infer opponent last move from board snapshots (tournament-safe; no history required).

We store a deep copy of the board at the start of each of our turns plus our chosen move.
At the next turn, we replay our move + perspective swap, then match the opponent's turn
by enumerating legal moves (including search) against the observed board.
"""
...
def infer_opponent_category(
    ...
    """
    Return opponent move category between our last turn and this turn, or None if unknown.
    """
    ...
    # Opponent search channel updated after their turn; compare to snapshot at our last start.
    ...
    for mv in candidates:
        ...
        if mv.move_type == MoveType.SEARCH:
            # apply_move does not apply search scoring; trust observed points.
        ...
        # Search channels are a rolling two-turn telemetry deque updated around perspective swaps.
        # For non-search classification, state structure (board masks/positions/points/turn/perspective)
        # is the stable discriminator; channel equality can fail even when move replay is otherwise exact.

...
class TimeManager:
    ...
    @staticmethod
    def phase_multiplier(turn_count: int, state: RuntimeState | None = None) -> float:
        if turn_count < 20:
            return 1.80 if state is None else state.time_opening_multiplier
        if turn_count < 60:
            return 1.40 if state is None else state.time_mid_multiplier
        return 1.00 if state is None else state.time_late_multiplier

    @staticmethod
    def phase_cap(turn_count: int, state: RuntimeState | None = None) -> float:
        if turn_count < 20:
            return 8.0 if state is None else state.time_opening_cap
        if turn_count < 60:
            return 6.0 if state is None else state.time_mid_cap
        return 3.0 if state is None else state.time_late_cap
    ...
    def allocation(cls, board: Board, state: RuntimeState, time_remaining: float) -> tuple[float, bool]:
        ...
        alloc_raw = base * cls.phase_multiplier(board.turn_count, state)
        alloc_raw = max(cls.MIN_TURN_BUDGET, min(alloc_raw, cls.phase_cap(board.turn_count, state)))

...
class PolicyEngine:
        ...
        *,
        ...
        reject = bool(
            ...
            and (
                ...
                or survival < 0.35
                or counter_threat >= guaranteed + 2.0
            )
        )
    ...
    def _search_gate(
        ...
        if state.farmable_rat:
            q_best_non *= 0.90
            guess_margin = 0.18 + 0.18 * turn_fraction + lead_adjustment
        else:
            guess_margin = 0.30 + 0.45 * turn_fraction + lead_adjustment
        ...
        search_value = best_search_sv + 0.5 * p1
    ...
    def score_non_search(
        ...
        *,
        ...
        if action.move_type == MoveType.CARPET:
            ...
            total -= 1.1 * float(carpet_profile["risk"])

        continuation = self._continuation_tiebreak(action, state)
        return total, {
            "mobility": float(exit_count),
            "immediate": immediate,
            "risk": risk + (float(carpet_profile["risk"]) if action.move_type == MoveType.CARPET else 0.0),
    ...
    def _recovery_filter_non_search(
        ...
        return safe_value + neutral + setup_value if (safe_value or neutral) else moves

...
def _popcount(mask: int) -> int:
    return (mask & ((1 << NUM_CELLS) - 1)).bit_count()
...
class BBState:
    ...
    def from_board(board: Board) -> "BBState":
        ...
        return BBState(
            ...
            us_to_move=True,
            hash=0,  # to be filled by caller using zobrist_hash()
        )
...
def _swap_sides(state: BBState, keys: ZobristKeys) -> BBState:
    """Swap perspective after a move: the old opponent becomes `us`.

    Update the incremental hash for:
    - Side-to-move toggle.
    - Worker position swap (us_worker <-> opp_worker keys).
    - Score band swap (us_band <-> opp_band keys).
    """
    h = state.hash
    # 1. Toggle side to move.
    h ^= keys.side_to_move
    # 2. Swap worker positions in hash (XOR out old, XOR in swapped).
    h ^= keys.us_worker[state.us] ^ keys.opp_worker[state.us]
    h ^= keys.opp_worker[state.opp] ^ keys.us_worker[state.opp]
    # 3. Swap score bands in hash.
    h ^= keys.us_score_band[state.us_score % 32] ^ keys.opp_score_band[state.us_score % 32]
    h ^= keys.opp_score_band[state.opp_score % 32] ^ keys.us_score_band[state.opp_score % 32]
...
def _swap_sides_null(state: BBState, keys: ZobristKeys) -> BBState:
    """Swap perspective WITHOUT consuming a turn (for NMP)."""
    h = state.hash
    h ^= keys.side_to_move
    h ^= keys.us_worker[state.us] ^ keys.opp_worker[state.us]
    h ^= keys.opp_worker[state.opp] ^ keys.us_worker[state.opp]
    ...
    return BBState(
        ...
        opp_score=state.us_score,
        us_turns=state.opp_turns,
        opp_turns=state.us_turns, # no turn reduction
        us_to_move=not state.us_to_move,
        hash=h,
    )


def apply_move_key(state: BBState, key: MoveKey, keys: ZobristKeys) -> BBState:
    ...
    if mt == 0:  # PLAIN
        ...
        # Hash update: XOR out old us_worker pos, XOR in new.
        h ^= keys.us_worker[us_idx] ^ keys.us_worker[nb]
        ...
        return _swap_sides(new_state, keys)
    ...
    if mt == 1:  # PRIME
        ...
        # Hash updates:
        # - Move worker: XOR out old us_worker pos, XOR in new.
        h ^= keys.us_worker[us_idx] ^ keys.us_worker[nb]
        # - Turn cell space -> primed:
        h ^= keys.slot_cell[0][us_idx]  # SPACE off
        h ^= keys.slot_cell[1][us_idx]  # PRIMED on
        # - Update score:
        h ^= keys.us_score_band[us_score % 32]
        new_us_score = us_score + 1
        h ^= keys.us_score_band[new_us_score % 32]
    ...
    if mt == 2:  # CARPET
        ...
        for _ in range(b):
            ...
            new_carpet |= cell_bit
            final = cur
            # Hash: PRIMED off, CARPET on.
            h ^= keys.slot_cell[1][cur]
            h ^= keys.slot_cell[2][cur]

        # Hash: update worker position.
        h ^= keys.us_worker[us_idx] ^ keys.us_worker[final]

        points = CARPET_POINTS_LUT[b] if 0 <= b < len(CARPET_POINTS_LUT) else -10_000
        # Hash: update score.
        h ^= keys.us_score_band[us_score % 32]
        new_us_score = us_score + points
        h ^= keys.us_score_band[new_us_score % 32]

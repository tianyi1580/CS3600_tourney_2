...
class PolicyEngine:
    ...
    @staticmethod
    def _count_space_components(board: Board) -> int:
        """Count connected components of SPACE cells via flood-fill.

        A cell is SPACE if it is not blocked, primed, carpet, or occupied by a worker.
        Two SPACE cells are connected if they are cardinal-adjacent.
        Returns the number of connected components (0 if no SPACE cells).
        """
        visited: set[tuple[int, int]] = set()
        worker_locs = {board.player_worker.get_location(), board.opponent_worker.get_location()}
        components = 0

        for y in range(BOARD_SIZE):
            for x in range(BOARD_SIZE):
                loc = (x, y)
                if loc in visited or loc in worker_locs:
                    continue
                cell = board.get_cell(loc)
                if cell.name != "SPACE":
                    continue
                # BFS flood-fill from this SPACE cell
                components += 1
                stack = [loc]
                visited.add(loc)
                while stack:
                    ...
                    for dx, dy in ((0, 1), (0, -1), (1, 0), (-1, 0)):
                        nx, ny = cx + dx, cy + dy
                        nloc = (nx, ny)
                        if nloc in visited or nloc in worker_locs:
                            continue
                        if not board.is_valid_cell(nloc):
                            continue
                        if board.get_cell(nloc).name == "SPACE":
                            visited.add(nloc)
                            stack.append(nloc)
        return components

    def _fragmentation_score(self, board_before: Board, board_after: Board, action: Move) -> float:
        ...
        if action.move_type != MoveType.CARPET:
            return 0.0
        comp_before = self._count_space_components(board_before)
        comp_after = self._count_space_components(board_after)
    ...
    def _entry_chain_lengths(
        ...
        *,
        ...
        for y in range(BOARD_SIZE):
            for x in range(BOARD_SIZE):
                loc = (x, y)
                if not self._is_walkable_plain_cell(board, loc, include_occupied=include_occupied):
                    continue
                best_k = 0
                for direction in (Direction.UP, Direction.DOWN, Direction.LEFT, Direction.RIGHT):
                    k = self._adjacent_primed_chain_len(board, loc, direction)
                    if k >= min_chain_len:
                        best_k = max(best_k, k)
                if best_k >= min_chain_len:
                    entries[loc] = best_k
        return entries

    def _shortest_walkable_distance_to_entry(
        self, board: Board, start: tuple[int, int], entries: set[tuple[int, int]]
    ) -> int | None:
        """BFS shortest path through plain-walkable cells to any entry cell."""
        if not entries:
            return None
        if start in entries:
            return 0
        ...
        idx = 0
        while idx < len(queue):
            (cx, cy), dist = queue[idx]
            idx += 1
            for dx, dy in ((0, 1), (0, -1), (1, 0), (-1, 0)):
                nxt = (cx + dx, cy + dy)
                if nxt in visited:
                    continue
                if not self._is_walkable_plain_cell(board, nxt):
                    continue
                if nxt in entries:
                    return dist + 1
                visited.add(nxt)
    ...
    def _reachable_best_chain_value(self, board: Board, start: tuple[int, int], min_chain_len: int = 3) -> float:
        """Best carpet value among reachable entry cells."""
        entries = self._entry_chain_lengths(board, min_chain_len=min_chain_len)
        if not entries:
            return 0.0
        reachable_best = 0.0
        ...
        while idx < len(queue):
            ...
            idx += 1
            if cur in entries:
                reachable_best = max(reachable_best, float(CARPET_POINTS_TABLE[min(entries[cur], 7)]))
            cx, cy = cur
            for dx, dy in ((0, 1), (0, -1), (1, 0), (-1, 0)):
                nxt = (cx + dx, cy + dy)
                if nxt in visited:
    ...
    def _phase_multipliers(phase: str) -> tuple[float, float, float, float, float]:
        ...
        if phase == "opening":
            return (0.9, 1.3, 1.2, 0.5, 1.2)
        if phase == "mid":
            return (1.0, 1.0, 1.3, 1.0, 1.0)
        return (1.3, 0.5, 0.4, 1.5, 0.6)
    ...
    def _prime_direction_bonus(self, board: Board, action: Move, dest: tuple[int, int]) -> float:
        ...
        # w_chain=3.0, w_align=2.5, w_opp=0.3 — normalised to [0, ~5]
        return max(0.0, (3.0 * ahead + 2.5 * alignment - 0.3 * opp_proximity) / 5.5)
    ...
    def _nearest_primable_corridor_distance(self, board: Board, start: tuple[int, int]) -> int | None:
        """BFS distance to any location with at least one valid prime move."""
        if not self._is_walkable_plain_cell(board, start, include_occupied=True):
            return None

        def has_prime_from(loc: tuple[int, int]) -> bool:
            if not self._is_walkable_plain_cell(board, loc, include_occupied=True):
                return False
            if board.get_cell(loc).name != "SPACE":
                return False
            opp = board.opponent_worker.get_location()
        ...
        while idx < len(queue):
            ...
            for nxt in self._neighbors(cur):
                ...
                if not self._is_walkable_plain_cell(board, nxt):
                    continue
                if has_prime_from(nxt):
                    return dist + 1
                visited.add(nxt)
                queue.append((nxt, dist + 1))
        return None

    def _no_good_move_fallback(self, board: Board, candidates: list[Move]) -> Move:
        ...
        if plains:
            ...
            base_dist = self._nearest_primable_corridor_distance(board, current)
            scored: list[tuple[int, tuple, Move]] = []
            for m in plains:
                dest = self._destination(board, m)
                d = self._nearest_primable_corridor_distance(board, dest)
    ...
    def _carpet_landing_adjustment(self, board_after: Board, action: Move, dest: tuple[int, int]) -> float:
        """Reward carpet landings that preserve immediate mobility and prime restart options (§3.5)."""
        if action.move_type != MoveType.CARPET:
            return 0.0
        exits = self._count_plain_exits(board_after, dest)
        has_space_exit = any(
            board_after.is_valid_cell(nxt)
            and board_after.get_cell(nxt).name == "SPACE"
            and self._is_walkable_plain_cell(board_after, nxt)
            for nxt in self._neighbors(dest)
        )
        bonus = 0.0
    ...
    def _high_value_deny_bonus(self, board: Board, action: Move, dest: tuple[int, int]) -> float:
        ...
        entries = self._entry_chain_lengths(board, min_chain_len=5, include_occupied=True)
        if not entries:
            return 0.0
        ...
        for nxt in self._neighbors(opp):
            if nxt in entries and self._is_walkable_plain_cell(board, nxt):
                threatened.add(nxt)
    ...
    def _prime_adjacent_opponent_blocks_approach(
        ...
        before_entries = set(self._entry_chain_lengths(board, min_chain_len=3).keys())
        if not before_entries:
            return False
        after_entries = set(self._entry_chain_lengths(board_after, min_chain_len=3).keys())

        before_dist = self._shortest_walkable_distance_to_entry(board, src, before_entries)
        after_dist = self._shortest_walkable_distance_to_entry(board_after, dest, after_entries)
    ...
    def _identify_opponent_chain_entries(self, board: Board) -> set[tuple[int, int]]:
        """Find cells that the opponent can use to roll a carpet of length 3 or more."""
        threats: set[tuple[int, int]] = set()
        opp_loc = board.opponent_worker.get_location()
        
        # Look for existing primed chains on the board
        for y in range(BOARD_SIZE):
            for x in range(BOARD_SIZE):
                loc = (x, y)
                if board.get_cell(loc).name == "PRIMED":
                    # For every primed cell, check all 4 directions to find the 'entry' cells
                    for d in (Direction.UP, Direction.DOWN, Direction.LEFT, Direction.RIGHT):
                        # Walk backwards to find the potential entry cell
                        rev = _REVERSE_DIR[d]
                        cur = loc
                        chain_len = 1
                        # Follow the chain
                        while True:
                            ...
                            if not board.is_valid_cell(nxt) or board.get_cell(nxt).name != "PRIMED":
                                break
                            cur = nxt
                            chain_len += 1
                        
                        if chain_len >= 2:
                            # The cell before the start of this chain is a potential entry point
                            entry = loc_after_direction(loc, rev)
                            if board.is_valid_cell(entry):
                                threats.add(entry)
                            # Any cell within the chain is also a target to 'break' it
                            # (already covered by checking every PRIMED cell)
                            threats.add(loc)
        return threats

    def score_non_search(
        self, board: Board, action: Move, belief: BeliefEngine, state: RuntimeState
        ...
        if action.move_type == MoveType.PRIME:
            ...
            before_best = self._reachable_best_chain_value(board, src_before, min_chain_len=3)
            after_best = self._reachable_best_chain_value(board_after, dest, min_chain_len=3)
            ...
            if self._prime_adjacent_opponent_blocks_approach(board, board_after, src_before, dest):
                return float("-inf"), {"mobility": float(exit_count), "immediate": immediate, "risk": 1.3, "continuation": 0.0}
        ...
        # Yolanda_Mitchell: Avoid center in mid-game if leading, else fight for it.
        if phase == "opening":
            centrality_scale = 0.5
        elif phase == "mid":
            if has_lead:
                centrality_scale = -1.5 # Toned down center avoidance
            else:
                centrality_scale = 1.0  # Normal behavior when trailing or tied
        else:
            centrality_scale = 0.3

        position = (exit_count / 4.0) + 0.3 * centrality * centrality_scale

        # Yolanda_Mitchell: Bonus for moving to SPACE (unexplored) cells in mid-game if leading
        if phase == "mid" and has_lead and board.get_cell(dest).name == "SPACE":
            position += 1.0

        # §3.3 / §6.4: use walkable-path distance (BFS), not Manhattan distance.
        if action.move_type == MoveType.PLAIN:
            entry_map = self._entry_chain_lengths(board_after, min_chain_len=3)
            steps_to_entry = self._shortest_walkable_distance_to_entry(board_after, dest, set(entry_map.keys()))
            if steps_to_entry is not None and steps_to_entry <= 2:
                position += 0.8
            elif steps_to_entry is None:
                position -= 0.8
            else:
                position -= 0.6

        carpet_setup = self._chain_score(board_after, dest)
        carpet_setup += self._prime_direction_bonus(board, action, dest)

        # Yolanda_Mitchell: Sabotage Mode (Aggression when Leading)
        # Crank up the denial coefficient and add specific sabotage rewards
        sabotage_bonus = 0.0
        if has_lead:
            # We have the lead, so our goal is to maintain it by stopping them.
            state.effective_d *= 2.5 # Drastically increase generic denial
            
            opp_threats = self._identify_opponent_chain_entries(board)
            if dest in opp_threats:
                sabotage_bonus = 10.0 # Huge reward for breaking/blocking their chains
        ...
        denial += self._high_value_deny_bonus(board, action, dest)
        denial += sabotage_bonus

        risk = self._risk_score(exit_count)
        risk += self._corridor_trap_penalty(board_after, dest, exit_count)

        fragmentation = self._fragmentation_score(board, board_after, action)
        ...
        # Yolanda_Mitchell: Threatened Chain Protection
        # If we have a decent chain (L4+) and the opponent is within 2 steps, take it NOW.
        if action.move_type == MoveType.CARPET and action.roll_length >= 4:
            src = board.player_worker.get_location()
            opp_loc = board.opponent_worker.get_location()
            
            # Check if any part of the primed chain is threatened
            d = action.direction
            k = action.roll_length
            ...
            for _ in range(k):
                cur = loc_after_direction(cur, d)
                dist_to_opp = abs(cur[0] - opp_loc[0]) + abs(cur[1] - opp_loc[1])
                if dist_to_opp <= 2:
                    chain_threatened = True
                    break
            
            if chain_threatened:
                timing_adj += 10.0 # Significant boost to cash out before it's broken

        # Yolanda_Mitchell: Take long carpets early no matter what
        if phase == "opening" and action.move_type == MoveType.CARPET and action.roll_length >= 5:
            timing_adj += 50.0
    ...
    def _late_phase_filter_non_search(self, board: Board, moves: list[Move]) -> list[Move]:
        ...
        if plains:
            entries = set(self._entry_chain_lengths(board, min_chain_len=3).keys())
            if entries:
                src = board.player_worker.get_location()
                src_d = self._shortest_walkable_distance_to_entry(board, src, entries)
                closer: list[Move] = []
                for m in plains:
                    d = self._shortest_walkable_distance_to_entry(board, self._destination(board, m), entries)
    ...
    def select_action(
        ...
        # Yolanda_Mitchell: Tactical Fast Search
        # Only "gamble" on the rat if we don't have a high-value carpet ready to roll.
        best_immediate_carpet = 0
        for mv in board.get_valid_moves(exclude_search=True):
            if mv.move_type == MoveType.CARPET:
                best_immediate_carpet = max(best_immediate_carpet, CARPET_POINTS_TABLE[mv.roll_length])

        top_k = belief.topk(1)
        if top_k and best_immediate_carpet < 10: # Don't skip a 10+ point carpet for a guess
            top_loc, top_prob = top_k[0]
            if top_prob > 0.85: # Increased threshold slightly for "Tactical Aggression"
                smove = Move.search(top_loc)
                if board.is_valid_move(smove):
                    state.snapshot_at_our_turn_start = board.get_copy()
                    state.last_own_move = smove
                    state.fallback_move = smove
        ...
        # Yolanda_Mitchell: Removed 'avoid early carpets' logic to prioritize long streaks early.
        ...
        eval_moves = sorted(non_search_candidates, key=self.move_sort_key) + sorted(
            search_candidates, key=self.move_sort_key
        )

        for mv in eval_moves:
            ...
            elif non_corridor_exists and corridor_flags.get(mv_key, False):
                tv, meta = float("-inf"), {
                    "mobility": 0.0,
                    "immediate": 0.0,
                    "risk": 1.3,
            ...
            if tv > best_non_search_tv or (
                tv == best_non_search_tv
                and (
                    meta["mobility"],
                    meta["immediate"],
                    -meta["risk"],
        ...
        if best_non_search is None or best_non_search_tv <= 0.0:
            if best_search_sv > 0.0:
                return _persist_return(best_search)
            return _persist_return(self._no_good_move_fallback(board, non_search_candidates))

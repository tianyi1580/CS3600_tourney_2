"""
Bounded opponent adaptation (bot_plan_v4 Opponent Modeling + clamp contracts).
"""
...
# Rolling window for opponent action histogram + mobility
OPPONENT_WINDOW = 24

# Base coefficients (M2 defaults, policy v4)
BASE_A = 1.00
BASE_B = 0.45
BASE_C = 0.60
BASE_D = 0.35
...
# Per-parameter delta clamps (after confidence scaling)
D_CLAMP_A = (-0.10, 0.10)
D_CLAMP_C = (-0.10, 0.10)
D_CLAMP_D = (-0.15, 0.15)
D_CLAMP_F = (-0.10, 0.10)
...
def normalized_entropy_category_counts(counts: dict[OpponentCategory, int]) -> float:
    """Return H / H_max over four categories; 0 if empty."""
...
def pattern_raw_deltas(
    *,
    ...
    """Aggregate v4 adaptive weighting table rows (additive)."""
    r = RawDeltas()
    if total_typed <= 0:
        return r
    ...
    non_search_typed = (
        category_hist.get(OpponentCategory.PLAIN, 0)
        + category_hist.get(OpponentCategory.PRIME, 0)
        + category_hist.get(OpponentCategory.CARPET, 0)
    )
    ...
    if prime_frac >= 0.38 or (prime_frac >= 0.30 and prime_frac >= carpet_frac + 0.1 and prime_frac >= search_frac + 0.1):
        r.df += 0.10
...
def apply_adaptation(
    ...
    """
    Returns (effective_a, effective_c, effective_d, effective_f).
    b and e stay at policy defaults (not adapted in v4 table).
    """

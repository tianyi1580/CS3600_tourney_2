...
def plan_best_carpet_build(
    ...
    *,
    ...
    Total turns consumed: approach + k + 1. Total score: (k−1) prime points (for
    primes #2..#k; the first prime is on `s`) + CARPET_POINTS[k]. Actually each
    prime earns +1, so `k` primes earn `k` points, plus the carpet yield.
    ...
    for start_cell, approach in reach.items():
        if not (space_mask & (1 << start_cell)) and start_cell != state.us:
            continue
        # Must have own(start) >= 0. start == us may already be primed/carpet
        # — skip if not space.
        if not (space_mask & (1 << start_cell)) and start_cell == state.us:
            # Our cell is primed/carpet already; can't start a build here.
            continue
        if own[start_cell] < 0:
            continue

        # Try each direction; collect contiguous SPACE cells we could prime.
        for d in range(NUM_DIR):
            ray = RAY_SEQ[start_cell][d]
            safe_ray: list[int] = [start_cell]
            for cell in ray:
                cbit = 1 << cell
                ...
                if own[cell] < 0 and (d_opp[cell] - territory.d_us[cell] <= safe_margin):
                    break
                safe_ray.append(cell)
                if len(safe_ray) >= BOARD_SIZE - 1:
                    break

            if len(safe_ray) < 2:
                continue

            for k in range(2, len(safe_ray) + 1):
                # Consumes approach + k (primes) + 1 (carpet) turns.
                cost_turns = approach + k + 1
                if cost_turns > state.us_turns:
                    break
                # Safety: final cell must still be reachable-faster-than-opp.
                last_cell = safe_ray[k - 1]
...
def _plan_best_elbow(
    ...
    for start_cell, approach in reach.items():
        ...
        for d in range(NUM_DIR):
            ...
            for k1 in range(1, len(leg1)):
                # After priming k1 cells, worker is on leg1[k1 - 1 + 1] ... actually
                # the rule: when we PRIME, we set current cell to primed and STEP
                # to the next. After k1 primes starting on leg1[0], worker is on
                # leg1[k1].
                if k1 >= len(leg1):
                    break
                pivot_cell = leg1[k1]

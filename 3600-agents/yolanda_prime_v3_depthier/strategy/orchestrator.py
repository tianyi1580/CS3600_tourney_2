...
from .territory import compute_territory, prime_potential_array
...
class Orchestrator:
    ...
    def __init__(self, weights: dict, *, enable_debug: bool = False):
        ...
        self._zobrist_cache_key: Optional[bytes] = None
        self._zobrist = None
    ...
    def select_action(
        ...
        *,
        ...
        if isinstance(sensor_data, tuple) and len(sensor_data) == 2:
            ...
            runtime.use_single_step = False
            runtime.opp_miss_cell = None
            belief.update(noise, estimated_distance, board)
        ...
        # 7. Leaf evaluator.
        leaf_eval = build_leaf_eval(self.weights, prime_pot_field, belief_info_fn)

        # 8. Time allocation.
        signals = ComplexitySignals(
            root_branching=0,  # filled after generate
            top2_gap=0.0,
        ...
        if emergency:
            ...
            result = searcher.iterative_deepening(
                state,
                soft_deadline=start + 0.05,
                hard_deadline=start + 0.08,
                max_depth=1,
        ...
        # Critical: decide_search compares `ev_search` (expected incremental
        # points from firing SEARCH this turn, range ~[-2, +4]) against
        # `ev_best_move_non_search`. That means the non-search EV MUST also be
        # expressed as *incremental points this turn* — NOT the full alpha-beta
        # leaf-eval value (which mixes in the absolute score_delta and can be
        # strongly negative when we're behind, forcing the gate open on every
        # turn). We therefore derive the comparator from the concrete best move
        # the search chose, plus a small bonus for primes that immediately
        # unlock a long carpet-roll follow-up.
        q_best_non_search = _incremental_move_ev(state, best_move_key)
...
def _incremental_move_ev(state: BBState, mv_key) -> float:
    ...
    *plus* a small lookahead bonus for PRIME moves that immediately unlock a
    ...
    if mv_key[0] == 1:  # PRIME
        try:
            child = apply_move_key(state, mv_key)

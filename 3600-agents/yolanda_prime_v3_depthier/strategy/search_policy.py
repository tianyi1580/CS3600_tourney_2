...
def estimate_opp_search_threat(
    ...
    proxy_peak = max(runtime_opp_peak_proxy, max(0.0, 0.9 * p_us - 0.1))
    ...
    # Threat = P(opp_peak * sufficient_localization)
    threat_prob = proxy_peak
    return threat_prob, peak_cell
...
def decide_search(
    *,
    ...
    threat_prob, peak_cell = estimate_opp_search_threat(belief, board, opp_peak_proxy)
    denial_equity = 4.0 * threat_prob  # Hitting denies a 6 pt swing + removes 2 pt risk.
    ...
    # (We use ev_best_move_non_search rather than ev_best_move to avoid fighting
    # against a search EV that was itself computed by the search.)

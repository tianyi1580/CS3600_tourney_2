...
class PolicyEngine:
    ...
    def _chain_survival_summary(
        ...
        *,
        ...
        if min_dist <= 1:
            survival = 0.15 # Very defensive
        elif min_dist == 2:
            survival = 0.35 # Defensive
        elif min_dist == 3:
            survival = 0.60
        elif min_dist == 4:
            survival = 0.85
        else:
            survival = 0.98 # Very greedy
    ...
    def score_non_search(
        ...
        *,
        ...
        if action.move_type == MoveType.PRIME:
            ...
            # Priming should not walk into absolute dead-ends (exit_count == 0).
            if exit_count < 1:
    ...
    def select_action(
        ...
        if my_turns > 3:
            # Dynamic Search Valuation: only search if EV > best non-search move
            p1 = belief.topk(1)[0][1] if belief.topk(1) else 0.0
            search_ev = (p1 * 4.0) + ((1.0 - p1) * -2.0)
            
            if search_ev > best_non_search_heuristic_tv:
                return _persist_return(best_search)
            return _persist_return(best_non_search if best_non_search else self._best_safe_non_search(board))
        else:
            # Endgame Mode

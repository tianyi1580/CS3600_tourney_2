...
class PlayerAgent:
    ...
    def __init__(self, board, transition_matrix=None, time_left: Callable = None):
        ...
        self.policy_engine = PolicyEngine(
            enable_lead_aware_centrality=_env_flag("Y2_ENABLE_LEAD_AWARE_CENTRALITY", True),
            mid_lead_centrality_scale=_env_float("Y2_MID_LEAD_CENTRALITY_SCALE", 0.7),
            mid_trailing_centrality_scale=_env_float("Y2_MID_TRAILING_CENTRALITY_SCALE", 1.2),
            opening_centrality_scale=_env_float("Y2_OPENING_CENTRALITY_SCALE", 1.5),
            late_centrality_scale=_env_float("Y2_LATE_CENTRALITY_SCALE", 0.3),
            mid_lead_space_bonus=_env_float("Y2_MID_LEAD_SPACE_BONUS", 0.5),
            enable_threatened_cashout_bonus=_env_flag("Y2_ENABLE_THREATENED_CASHOUT", False),
            threatened_cashout_min_roll=max(1, _env_int("Y2_THREAT_CASHOUT_MIN_ROLL", 4)),
            threatened_cashout_opp_dist=max(0, _env_int("Y2_THREAT_CASHOUT_OPP_DIST", 3)),
            threatened_cashout_bonus=_env_float("Y2_THREAT_CASHOUT_BONUS", 2.0),
            enable_opponent_chain_sabotage=_env_flag("Y2_ENABLE_SABOTAGE", True),
            sabotage_min_chain_len=max(1, _env_int("Y2_SABOTAGE_MIN_CHAIN_LEN", 3)),
            sabotage_opp_dist=max(0, _env_int("Y2_SABOTAGE_OPP_DIST", 1)),
            sabotage_bonus=_env_float("Y2_SABOTAGE_BONUS", 2.0),
            enable_fast_search_shortcut=_env_flag("Y2_ENABLE_FAST_SEARCH", False),
            fast_search_prob_threshold=_env_float("Y2_FAST_SEARCH_PROB", 0.8),
            fast_search_max_carpet_points=_env_float("Y2_FAST_SEARCH_MAX_CARPET", 12.0),
            enable_opening_long_carpet_exception=_env_flag("Y2_ENABLE_OPENING_LONG_CARPET", True),
            opening_keep_min_roll=max(1, _env_int("Y2_OPENING_KEEP_MIN_ROLL", 4)),
        )
    ...
    def commentate(self):
        rs = self.runtime_state
        return (
            "Yolanda2 v2: Yolanda1 core + empirically tuned Mitchell fusion "

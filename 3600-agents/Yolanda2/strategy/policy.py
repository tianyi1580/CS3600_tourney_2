...
class PolicyEngine:
    ...
    def __init__(
        ...
        sabotage_opp_dist: int = 1,
        sabotage_bonus: float = 2.0,
        enable_fast_search_shortcut: bool = False,
        fast_search_prob_threshold: float = 0.8,
        fast_search_max_carpet_points: float = 12.0,
        enable_opening_long_carpet_exception: bool = True,
        opening_keep_min_roll: int = 4,
    ) -> None:
        ...
        self.enable_opening_long_carpet_exception = enable_opening_long_carpet_exception
        self.opening_keep_min_roll = opening_keep_min_roll
    ...
    def score_non_search(
        ...
        *,
        ...
        sabotage_bonus = 0.0
        if has_lead and self.enable_opponent_chain_sabotage:
    ...
    def select_action(
        ...
        if phase == "opening":
            if self.enable_opening_long_carpet_exception:
                non_short_carpet = [
                    m
                    for m in non_search_candidates
                    if m.move_type != MoveType.CARPET or m.roll_length >= self.opening_keep_min_roll
                ]
                if non_short_carpet:
                    non_search_candidates = non_short_carpet
            else:

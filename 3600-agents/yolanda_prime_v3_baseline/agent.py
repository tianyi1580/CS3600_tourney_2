"""Yolanda Prime v3 agent entrypoint.
...
class ConfiguredPlayerAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self.orchestrator = Orchestrator(weights=weights, enable_debug=_env_flag("YP3_DEBUG", False))
...
class PlayerAgent(ConfiguredPlayerAgent):
    """Frozen-weight control for hyperopt v3: package ``weights.json`` unless
    ``YP3_BASELINE_WEIGHTS_JSON`` is set.

    Uses a separate env channel from the tunable candidate
    (``yolanda_prime_v3`` / ``YP3_WEIGHTS_JSON``) so ladder games against this
    ...
    def __init__(self, board, transition_matrix=None, time_left: Callable | None = None):
        super().__init__(
            ...
            transition_matrix,
            time_left,
            weight_env_var="YP3_BASELINE_WEIGHTS_JSON",
            commentate_name="yolanda_prime_v3_baseline",
        )

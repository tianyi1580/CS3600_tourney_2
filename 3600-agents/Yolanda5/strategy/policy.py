...
class PolicyEngine:
    ...
    def select_action(
        ...
        search_deadline = turn_deadline - TimeManager.expensive_work_cushion(state)
        ...
        deep_eval_enabled, deep_top_k = TimeManager.deep_path_plan(
            board,
            state,
            now,
            max(0.0, alloc - TimeManager.expensive_work_cushion(state)),
        )
        ...
        if my_turns > 3 and filtered_non_search_candidates:
            lookahead_budget = TimeManager.tactical_search_budget(
                board,
                state,

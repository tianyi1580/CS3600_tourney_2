# Deep Research Plan v4 (Ground-Truth Complete, Decision-Complete)
...
3. **Update belief**: If a prior successful search (own or observed opponent via search channels) implies rat respawn, reset belief to cached post-spawn prior. Otherwise, perform prediction step (`B = B @ T`), compute observation likelihoods from `noise` (cell-type emission table) and `estimated_distance` (offset distribution with `max(0,...)` clipping), apply element-wise update, and normalize to sum `1.0`. `[Required by engine]`
...
5. **Score and compare**: Score all non-search candidates using tactical scorer `S_total` (see Scoring Component Definitions). Score search candidates using `EV_search`. Apply normalized cross-family comparison and search margin schedule from Policy Constants Table. `[Policy default in v4]`
...
| Search normalization rule | Search EV and non-search tactical score are different scales | Normalize with deterministic online moments (Welford): `EVn = (EV - mu_ev) / max(sigma_ev, eps)`, `Tn = (T - mu_t) / max(sigma_t, eps)`. Bootstrap constants before sample floor: `mu_ev=0`, `sigma_ev=1`, `mu_t=0`, `sigma_t=1`, `eps=1e-6` | Compare action ordering under controlled distributions | Cross-family comparison remains stable and finite even before calibration data accumulates | `[Policy default in v4]` |
...
| Search margin schedule | Engine does not prescribe threshold policy | Trigger search if `EVn_best >= Tn_best + margin(state)` where margin default is: trailing `-0.10`, tied `0.00`, leading `+0.10`; add `+0.15` when `t < 8s` | Scenario matrix across score/time regimes | Trigger decisions follow deterministic table | `[Policy default in v4]` |
...
| Search success rat reset parity | On successful search, rat respawns from `(0,0)` plus `1000` hidden moves | After successful search, reset belief to post-spawn prior computed as `e_0 @ numpy.linalg.matrix_power(T, 1000)` where `e_0` is one-hot at flat index `0` and `T` is the constructor-provided transition matrix. Cache this vector in `__init__` for O(1) reuse on every capture reset. | Controlled belief test with forced successful search event | Post-search prior matches cached vector; vector sums to `1.0`; vector is reused identically on every capture reset | §2.5, §6.3, §10.4 |

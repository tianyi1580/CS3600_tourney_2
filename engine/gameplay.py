...
# JAX is optional. The engine forks subprocesses; importing JAX (multi-threaded)
# before forking can deadlock. Default: do NOT import/use JAX.
# Set ENGINE_USE_JAX=1 to opt in.
if os.environ.get("ENGINE_USE_JAX", "0") == "1":
    try:
...
def _load_transition_matrix():
    ...
    # Fallback (no JAX): NumPy implementation.
    T = _np.asarray(T, dtype=_np.float32)
    noise = _np.random.default_rng(random.randint(0, 2**32 - 1)).uniform(-0.1, 0.1, size=T.shape)
    T = _np.maximum(T * (1.0 + noise), 0.0)
    row_sum = T.sum(axis=1, keepdims=True)
    row_sum = _np.where(row_sum == 0, 1.0, row_sum)
    return T / row_sum
...
def play_game(
    ...
    if play_time_override is not None:
        play_time = int(play_time_override)

#!/usr/bin/env python3
"""Offline black-box optimizer for ``yolanda_prime_v4_7`` weights.

Mirrors :mod:`scripts.optimize_yp2_weights` but retargets the v3 agent, env
channel (``YP4_WEIGHTS_JSON``), output directory
(``data/hyperopt/yolanda_prime_v4_7``), and the v3 opponent ladder
(:mod:`workflows.yp4_hyperopt`).
...
from workflows.yp4_hyperopt import (
    CANDIDATE_AGENT,
    CANDIDATE_ENV_VAR,
    DEFAULTS,
    PARAMETER_PROFILES,
...
DEFAULT_OUTPUT_DIR = ROOT / "data" / "hyperopt" / "yolanda_prime_v4_7"
...
def _verify_weight_channel_isolation() -> None:
    """Confirm that yolanda_prime_v4_7 loads candidate weights from YP4_WEIGHTS_JSON."""
    from yolanda_prime_v4_7.agent import PlayerAgent

    env = {
        "YP4_WEIGHTS_JSON": json.dumps({"alpha": 2.4, "time_reserve": 7.5}),
    }
    with _temporary_env(env):
        try:
            v4 = PlayerAgent(_build_preflight_board(), time_left=lambda: 10.0)
        except Exception as e:
            raise RuntimeError(f"Failed to initialize v4_7 agent in preflight: {e}")

    v4_alpha = float(getattr(v4, "weights", {}).get("alpha", float("nan")))
    if abs(v4_alpha - 2.4) > 1e-9:
        raise RuntimeError(
            f"yolanda_prime_v4_7 did not load weights from YP4_WEIGHTS_JSON (got alpha={v4_alpha})"
        )


def _preflight_stack(*, eval_profile: str, max_workers: int) -> None:
    """Fail fast on broken env channels or a dead evaluation harness."""

    _verify_weight_channel_isolation()
    # One-game parallel sanity ping against core ladder peers.
    preflight_opponents = (
    ...
    print(f"  [Preflight] Testing vs {preflight_opponents} in parallel...", flush=True)
    play_time, limit_resources = profile_settings(eval_profile)
    tasks = []
    for opponent_name in preflight_opponents:
        tasks.append(
            GameTask(
                candidate_weights_json=serialize_weights(DEFAULTS),
    ...
    for res in results:
        if res.catastrophic_loss:
            raise RuntimeError(
                f"Preflight series against {res.opponent_name} produced a candidate catastrophic loss"
            )
    ...
    *,
    ...
    best_evaluation = asdict(
        evaluate_fitness(
            ...
            baseline_weights=baseline_weights,
            workers_per_series=max_workers,
            pool_override=pool,
        )
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Offline black-box optimizer for yolanda_prime_v4_7 weights.")
    ...
    parser.add_argument(
        ...
        help="Optional JSON file for yp12 baseline-style regression self-play (optional for v3)",
    )
    ...
    parser.add_argument("--enable-mercy-rule", action="store_true", help="Enable multi-stage early pruning")
    parser.add_argument("--mercy-min-games", type=int, default=10)
    parser.add_argument("--mercy-floor-25", type=float, default=-8.0, help="Kill at 25% if avg_margin < this")
    parser.add_argument("--mercy-floor-50", type=float, default=-2.0, help="Kill at 50% if avg_margin < this")
    parser.add_argument("--mercy-prune-slow-leaders", action="store_true", default=True, help="Kill at 75% if lagging behind leader")
    parser.add_argument("--mercy-leader-delta", type=float, default=3.0, help="Lag distance for 75% pruning")
    parser.add_argument("--mercy-penalty", type=float, default=50.0)
    ...
    print(f"Output directory initialized: {output_dir.resolve()}", flush=True)
    ...
    mercy_cfg = MercyRuleConfig(
        ...
        floor_25=float(args.mercy_floor_25),
        floor_50=float(args.mercy_floor_50),
        prune_slow_leaders=bool(args.mercy_prune_slow_leaders),
        leader_margin_delta=float(args.mercy_leader_delta),
    ...
    if not args.skip_preflight:
        ...
        _preflight_stack(eval_profile=args.evaluation_profile, max_workers=args.workers)

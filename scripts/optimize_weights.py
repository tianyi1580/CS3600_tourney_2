...
from workflows.y4_hyperopt import (
    DEFAULTS,
...
from Yolanda4.infra.weights import PARAMETER_PROFILES

DEFAULT_OUTPUT_DIR = ROOT / "data" / "hyperopt" / "yolanda4"
...
def _load_json(path: str | None) -> dict[str, float]:
    if not path:
        return {}
...
def _verify_weight_channel_isolation() -> None:
    ...
    if abs(live_agent.policy_engine.a - 2.1) > 1e-9:
        raise RuntimeError("Yolanda4 did not load candidate weights from Y4_WEIGHTS_JSON")
    if abs(baseline_agent.policy_engine.a - 0.9) > 1e-9:
        raise RuntimeError("Yolanda4Baseline did not load baseline weights from Y4_BASELINE_WEIGHTS_JSON")
    if abs(TimeManager.phase_cap(0, baseline_agent.runtime_state) - 2.0) > 1e-9:
        raise RuntimeError("Yolanda4Baseline did not isolate baseline time-control weights")
...
def _preflight_stack(
    *,
    ...
    for opponent_name in ("RandomSearchBaseline", "Yolanda4Baseline"):
    ...
    *,
    ...
    for generation in range(start_gen, generations):
            ...
            with ProcessPoolExecutor(max_workers=max_workers) as pool:
                ...
                for idx, (candidate, future) in enumerate(zip(population, futures)):
                    try:
                        ...
                        print(f"  [ERROR] Candidate {idx} evaluation failed: {e}", flush=True)
        ...
        if strict_check_interval > 0 and ((generation + 1) % strict_check_interval == 0):
            print(f"  Performing strict mode validation...", flush=True)
        ...
        print(f"  Gen {generation} Summary: Best={generation_best_fitness:.3f} Mean={mean_fit:.3f} Worst={scored[-1][1]:.3f}", flush=True)
...
def _cma_optimize(
    *,
    ...
    while not es.stop() and generation < generations:
        ...
        print(f"  Gen {generation} Summary: Best={generation_best_fitness:.3f} Mean={mean_fit:.3f} Worst={float(np.min(fitnesses)):.3f}")

        strict_passed = None
        strict_details = None
        if strict_check_interval > 0 and ((generation + 1) % strict_check_interval == 0):
            print(f"  Performing strict mode validation...")
            strict_passed, strict_details = strict_mode_validation(generation_best_weights)
            status = "PASSED" if strict_passed else "FAILED"
            print(f"  [STRICT] {status}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Offline black-box optimizer for Yolanda4 weights.")
    parser.add_argument("--algo", choices=("auto", "cma", "ga"), default="auto")
    parser.add_argument("--profile", choices=tuple(PARAMETER_PROFILES.keys()), default="tier_ab", help="Weight subspace to optimize")
    parser.add_argument("--ladder", choices=("default", "smoke"), default="smoke", help="Opponent ladder preset")
    ...
    parser.add_argument("--baseline-weights-file", help="Optional JSON file used by Yolanda4Baseline during regression/self-play")

#!/usr/bin/env python3
"""Offline black-box optimizer for ``yolanda_prime_v3`` weights.

Mirrors :mod:`scripts.optimize_yp2_weights` but retargets the v3 agent, env
channel (``YP3_WEIGHTS_JSON``), output directory
(``data/hyperopt/yolanda_prime_v3``), and the v3 opponent ladder
(:mod:`workflows.yp3_hyperopt`).
...
from workflows.yp3_hyperopt import (
    CANDIDATE_AGENT,
    CANDIDATE_ENV_VAR,
    DEFAULTS,
    PARAMETER_PROFILES,
...
DEFAULT_OUTPUT_DIR = ROOT / "data" / "hyperopt" / "yolanda_prime_v3"
...
def _verify_weight_channel_isolation() -> None:
    """YP3_WEIGHTS_JSON must steer v3 without leaking into v2/yp12.

    v3 uses ``alpha`` (score_delta coefficient) as its tier-A anchor knob. We
    probe the channel by feeding a distinct alpha through YP3_WEIGHTS_JSON and
    confirming that (a) v3 reads it and (b) v2/yp12 ignore it entirely.
    """

    from yolanda_prime_v3.agent import PlayerAgent as V3Agent
    from yolanda_prime_v2.agent import PlayerAgent as V2Agent
    from yolanda_prime_v1_2.agent import PlayerAgent as V12Agent

    env = {
        "YP3_WEIGHTS_JSON": json.dumps({"alpha": 2.4, "time_reserve": 7.5}),
        "YP2_WEIGHTS_JSON": json.dumps({"a": 2.1}),
        "YP12_WEIGHTS_JSON": json.dumps({"a": 1.7}),
    }
    with _temporary_env(env):
        v3 = V3Agent(_build_preflight_board(), time_left=lambda: 10.0)
        v2 = V2Agent(_build_preflight_board(), time_left=lambda: 10.0)
        v12 = V12Agent(_build_preflight_board(), time_left=lambda: 10.0)

    v3_alpha = float(getattr(v3, "weights", {}).get("alpha", float("nan")))
    if abs(v3_alpha - 2.4) > 1e-9:
        raise RuntimeError(
            "yolanda_prime_v3 did not load candidate weights from YP3_WEIGHTS_JSON "
            f"(got alpha={v3_alpha})"
        )
    if abs(v2.policy_engine.a - 2.1) > 1e-9:
        raise RuntimeError(
            "yolanda_prime_v2 did not load its weights from YP2_WEIGHTS_JSON "
            f"(got a={v2.policy_engine.a}); possible env-var crosstalk"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Offline black-box optimizer for yolanda_prime_v3 weights.")

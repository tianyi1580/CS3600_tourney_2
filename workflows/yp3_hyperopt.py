"""Hyperopt pipeline for ``yolanda_prime_v3``.
...
``yolanda_prime_v3.infra.weights``.
...
+--------------------------------+---------+--------+--------------------------+
| yolanda_prime_v2               |   .30   |   70   | v2 regression anchor     |
...
from yolanda_prime_v3.infra.weights import (  # noqa: E402
    DEFAULTS,
    PARAMETER_PROFILES,
    bounds_for,
    clamp_weights,
...
CANDIDATE_AGENT: str = "yolanda_prime_v3"
CANDIDATE_ENV_VAR: str = "YP3_WEIGHTS_JSON"
# We use yolanda_prime_v2 (its frozen package weights) as the regression anchor
# because there is no yolanda_prime_v3_baseline snapshot yet. Once Stage-1
# converges, mint a v3_baseline package and swap the constant accordingly.
REGRESSION_OPPONENT: str = "yolanda_prime_v3_baseline"


DEFAULT_LADDER: tuple[OpponentConfig, ...] = (
    OpponentConfig("yolanda_prime_v2",          0.30, 21),
...
HYPEROPT_LADDER: tuple[OpponentConfig, ...] = (
    OpponentConfig("yolanda_prime_v3_baseline", 0.70, 60),
    OpponentConfig("yolanda_prime_v2",          0.30, 50),
)  # 110 games total across prioritized opponents

SMOKE_LADDER: tuple[OpponentConfig, ...] = (
    OpponentConfig("yolanda_prime_v2",          0.35, 2),
...
def run_single_game(task: GameTask) -> GameResult:
    ...
    # Candidate weights flow through ``YP3_WEIGHTS_JSON``. Because v3's
    # regression anchor is simply ``yolanda_prime_v2`` (using its own
    # package weights), we do NOT inject a baseline-weights channel here.
    # This field is reserved for a future ``yolanda_prime_v3_baseline``.
    env_updates: dict[str, str | None] = {
        CANDIDATE_ENV_VAR: task.candidate_weights_json,
    }
    if task.baseline_weights_json is not None:
        env_updates["YP3_BASELINE_WEIGHTS_JSON"] = task.baseline_weights_json

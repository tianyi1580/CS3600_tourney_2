...
from Yolanda4.infra.weights import (  # noqa: E402
    DEFAULTS,
    bounds_for,
    clamp_weights,
    parameter_names,
...
DEFAULT_LADDER: tuple[OpponentConfig, ...] = (
    ...
    OpponentConfig("Yolanda3_3", 0.225, 40),
    OpponentConfig("Yolanda4Baseline", 0.225, 40),
)
...
SMOKE_LADDER: tuple[OpponentConfig, ...] = (
    ...
    OpponentConfig("Yolanda3_3", 0.225, 2),
    OpponentConfig("Yolanda4Baseline", 0.225, 2),
)

STRICT_VALIDATION_OPPONENTS: tuple[str, ...] = ("yolanda_mitchell5", "yolanda_mitch1_2", "Yolanda3_3")

PROFILE_SETTINGS: dict[str, tuple[int, bool]] = {
    # Fast local stack validation without burning the full tournament budget.
    "smoke": (30, False),
    # 240s without Linux-only sandboxing is the right optimization default.
    "tuning": (240, False),
    "strict": (240, True),
    "local": (360, False),
}
...
def run_single_game(task: GameTask) -> GameResult:
    if task.opponent_name == "Yolanda4":
        raise ValueError("Use Yolanda4Baseline for regression/self-play checks; Yolanda4 vs Yolanda4 evaluates the candidate against itself.")

    env_updates = {
        "Y4_WEIGHTS_JSON": task.candidate_weights_json,
        "Y4_BASELINE_WEIGHTS_JSON": task.baseline_weights_json,
    }
    ...
    with temporary_env(env_updates):
        ...
        with redirectors:
            try:
                player_a = "Yolanda4" if task.candidate_is_a else task.opponent_name
                player_b = task.opponent_name if task.candidate_is_a else "Yolanda4"
